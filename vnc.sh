#! /bin/bash
sudo apt update && sudo DEBIAN_FRONTEND=noninteractive apt install -y tzdata npm keyboard-configuration xfce4 ubuntu-desktop lightdm dbus-x11 apt-utils sudo nano wget curl git dos2unix unzip xserver-xorg-video-dummy xserver-xorg-legacy x11vnc && sudo npm install -g localtunnel
...
startxfce4 &
x11vnc -display :0 -N -forever -shared -bg  &&   /opt/novnc/utils/novnc_proxy --web /opt/novnc --vnc localhost:5900 --listen 6080 &
sudo bash 6080.sh
